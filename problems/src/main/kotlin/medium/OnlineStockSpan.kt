...
class OnlineStockSpan {
    private val spanStack = Stack<Pair<Int, Int>>()

    fun next(price: Int): Int {
        var result = 1
        while (spanStack.isNotEmpty() && spanStack.peek().first <= price) {
            result += spanStack.pop().second
        }
        spanStack.push(price to result)
        return result
    }
}
