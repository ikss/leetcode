 ...
 * consecutive integers of a hidden sequence of length (n + 1). More formally, call the hidden sequence hidden, then we have that differences[i] = hidden[i + 1] - hidden[i].
 *
 ...
 */
object CountTheHiddenSequences {
    fun numberOfArrays(differences: IntArray, lower: Int, upper: Int): Int {
        var max = 0
        var min = 0
        var running = 0

        val givenDelta = upper - lower
        for (d in differences) {
            running += d
            max = maxOf(running, max)
            min = minOf(running, min)

            if (max - min > givenDelta) {
                return 0
            }
        }

        val delta = max - min
        return givenDelta - (delta - 1)
    }
}
