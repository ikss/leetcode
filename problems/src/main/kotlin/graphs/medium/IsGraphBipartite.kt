        ...
        for (i in graph.indices) {
            ...
            while (!queue.isEmpty()) {
                ...
                for (neighbour in graph[cur]) {
                    if (colors[neighbour] == 0) {          // If this node hasn't been colored, color it with different
                        colors[neighbour] = -colors[cur]
                        queue.offer(neighbour)
                    } else if (colors[neighbour] != -colors[cur]) {   // If it is colored with different color, return false
                        return false
                    }
                }
            }
